    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 overflow-y-auto py-20">
        ...
        <motion.div
          ...
          className="relative w-full max-w-lg bg-[#0a0a0a] border border-amber-500/20 rounded-[32px] overflow-hidden shadow-[0_0_50px_rgba(245,158,11,0.1)]"
        >
          {/* Header */}
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-amber-500 tracking-widest uppercase ml-1">Survey Type</label>
                  <select
                    value={formData.surveyType}
                    onChange={(e) => setFormData({ ...formData, surveyType: e.target.value })}
                    className="w-full bg-black/40 border border-white/10 rounded-xl py-4 px-5 text-white text-sm font-bold focus:outline-none focus:border-amber-500 transition-all appearance-none cursor-pointer"
                  >
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-amber-500 tracking-widest uppercase ml-1">Due Date</label>
                  <input
                    type="date"
                    required
                    value={formData.dueDate}
                    onChange={(e) => setFormData({ ...formData, dueDate: e.target.value })}
                    className="w-full bg-black/40 border border-white/10 rounded-xl py-4 px-5 text-white text-sm font-bold focus:outline-none focus:border-amber-500 transition-all"
                  />
                </div>
              </div>

              <div className="space-y-2">
                <label className="text-[10px] font-black text-amber-500 tracking-widest uppercase ml-1">Message / Instructions</label>
                <textarea
                  rows={4}
                  placeholder="EX: PLEASE COMPLETE THIS BEFORE YOUR 0900 SESSION..."
                  value={formData.instructions}
                  onChange={(e) => setFormData({ ...formData, instructions: e.target.value })}
                  className="w-full bg-black/40 border border-white/10 rounded-xl py-4 px-5 text-white text-sm font-medium placeholder:text-white/5 focus:outline-none focus:border-amber-500 transition-all resize-none"
                />
              </div>
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full bg-amber-500 text-black font-black uppercase tracking-[2px] py-5 rounded-xl flex items-center justify-center gap-2 hover:bg-amber-600 transition-all shadow-[0_10px_30px_rgba(245,158,11,0.3)] disabled:opacity-50"
              >
                {loading ? <Loader2 className="animate-spin" size={20} /> : "ASSIGN PROTOCOL SURVEY"}
