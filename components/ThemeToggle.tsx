...
export default function ThemeToggle({ variant = "pill" }: ThemeToggleProps) {
  ...
  if (!mounted) {
    ...
    return <div className="w-[52px] h-[28px] rounded-full bg-black/40 border border-white/10" />;
  }
  ...
  if (variant === "icon") {
    return (
      <button
        onClick={toggleTheme}
        className={`w-10 h-10 rounded-xl bg-white/5 border flex items-center justify-center transition-all duration-300 outline-none select-none active:scale-95 shrink-0 ${
          theme === "dark" 
            ? "border-white/10 text-white/40 hover:text-[#22c55e] hover:border-[#22c55e]/30" 
            : "border-[#00a855]/30 text-[#00a855] hover:text-[#00a855] hover:border-[#00a855]/50 bg-[#d4f5e2]/20"
        }`}
  ...
  return (
    <button
      ...
      className={`relative flex items-center w-[52px] h-[28px] rounded-full p-1 cursor-pointer transition-all duration-300 outline-none select-none hover:opacity-90 active:scale-95 shrink-0 ${
        theme === "dark" 
          ? "bg-black border border-white/10 shadow-[inset_0_1px_3px_rgba(0,0,0,0.8)]" 
          : "bg-[#d4f5e2] border border-[#00a855]/30 shadow-[inset_0_1px_3px_rgba(0,168,85,0.15)]"
      }`}
      aria-label={`Switch to ${theme === "dark" ? "light" : "dark"} mode`}
      title={`Switch to ${theme === "dark" ? "light" : "dark"} mode`}
    >
      ...
      <span
        className={`absolute w-5 h-5 rounded-full transition-transform duration-300 ease-in-out ${
          theme === "dark"
            ...
            : "translate-x-0 bg-[#00a855] shadow-[0_1px_3px_rgba(0,0,0,0.15),0_0_8px_rgba(0,168,85,0.4)]"
        }`}
      />
    </button>
  );
