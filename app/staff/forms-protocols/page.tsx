...
export default function StaffFormsAndProtocolsPage() {
  ...
  const isAuthorizedForProtocol = profile?.role === "superadmin" || profile?.role === "staff";

  return (
    <div className="space-y-6">
      {/* Page Title & Breadcrumbs handled in layout, but we show a header badge */}
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-[var(--bg-card)] border border-[var(--border-primary)] p-5 rounded-2xl">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] text-[var(--text-secondary)] uppercase tracking-widest mt-0.5">
              Document Management & Clinical Assessments
            </p>
          </div>
        </div>
        ...
        {!isAnyFormActive && (
          <div className="flex p-1 bg-[var(--bg-primary)] border border-[var(--border-primary)] rounded-xl w-full sm:w-auto">
            ...
            {/* Protocols Tab (visible to superadmin/staff only) */}
            {isAuthorizedForProtocol && (
              <button
      ...
      <DownloadSchedulesModal
        ...
        adminRedirectPath="/staff/curriculum"
      />
    </div>
  );
}
