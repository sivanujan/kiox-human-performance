...
(async () => {
  // Let's query profiles with role athlete just like the route does
  const { data: profilesData, error: athError } = await supabase
    .from('profiles')
    .select(`
  ...
  if (athError) {
    console.error('Error querying profiles:', athError);
  } else {
    console.log('Query profiles count:', profilesData ? profilesData.length : 0);
  }
})();
